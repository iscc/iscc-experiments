...
def prepare_images():
    ...
    for key, image in enumerate(os.listdir(IMAGE_SRC_DIR)):
        ...
        log.info(
            "Renamed {} -> {}".format(os.path.basename(src_path), os.path.basename(out))
        )
...
def generate_test_images():
    for name in os.listdir(IMAGE_SRC_DIR):
        ...
        bright_image.save(
            os.path.join(
                IMAGE_SRC_DIR, "%s_bright.%s" % (name_parts[0], name_parts[1])
            ),
            "PNG",
        )
        dark_image.save(
            os.path.join(IMAGE_SRC_DIR, "%s_dark.%s" % (name_parts[0], name_parts[1])),
            "PNG",
        )
        ...
        highContrast_image.save(
            os.path.join(
                IMAGE_SRC_DIR, "%s_highContrast.%s" % (name_parts[0], name_parts[1])
            ),
            "PNG",
        )
        lowContrast_image.save(
            os.path.join(
                IMAGE_SRC_DIR, "%s_lowContrast.%s" % (name_parts[0], name_parts[1])
            ),
            "PNG",
        )
        del enhancer
        ...
        cross_image.save(
            os.path.join(IMAGE_SRC_DIR, "%s_cross.%s" % (name_parts[0], name_parts[1])),
            "PNG",
        )
        del draw
        ...
        grey_image.save(
            os.path.join(IMAGE_SRC_DIR, "%s_grey.%s" % (name_parts[0], name_parts[1])),
            "PNG",
        )
        ...
        scaled_image.save(
            os.path.join(
                IMAGE_SRC_DIR, "%s_scaled.%s" % (name_parts[0], name_parts[1])
            ),
            "PNG",
        )
        ...
        crop_image = image.crop(
            (
                int(image.size[0] * random.uniform(0.01, 0.05)),
        ...
        crop_image.save(
            os.path.join(
                IMAGE_SRC_DIR, "%s_cropped.%s" % (name_parts[0], name_parts[1])
            ),
            "PNG",
        )
        ...
        gauss_image.save(
            os.path.join(IMAGE_SRC_DIR, "%s_gauss.%s" % (name_parts[0], name_parts[1])),
            "PNG",
        )
        ...
        if name_parts[1] == "jpg":
            ...
            image.save(
                os.path.join(
                    IMAGE_SRC_DIR, "%s_compressed.%s" % (name_parts[0], name_parts[1])
                ),
                "JPEG",
