...
def autolabel(rects, totals, bottom=None):
    ...
    for i, rect in enumerate(rects):
        ...
        plt.text(rect.get_x() + width / 2., bottom_height + 0.4 * height,
                 "%d\n(%s)" % (int(height), "{:.2%}".format(height / totals[i])), ha='center', va='bottom')
...
results = sorted(results, key=lambda k: (k["_source"]['bit_length'], k["_source"]['shingle_size']))
...
for result in results:
    ...
    names.append("Bit Length: %s\nShingle Size: %s" % (source["bit_length"], source["shingle_size"]))
